...
foreach (var (c, expectAgree, meaning) in cases)
{
    ...
    if (r.Agree == expectAgree)
    {
        ...
        if (c.Name.StartsWith("anim/", StringComparison.Ordinal))
        {
            Note("      " + AnimCases.Coverage());
        }

        if (c.Name.StartsWith("pane/", StringComparison.Ordinal) &&
            !c.Name.Equals("pane/header", StringComparison.Ordinal))
        {
        ...
        else if (c.Name.StartsWith("read/", StringComparison.Ordinal))
        {
        ...
        if (c.Name.StartsWith("subagents/", StringComparison.Ordinal))
        {
