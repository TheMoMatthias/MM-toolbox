...
if defined SR_GUI_SHOW (
    powershell.exe -STA -NoProfile -ExecutionPolicy Bypass -File "%~dp0lib\sessions-gui2.ps1" %*
    echo.
    pause
    exit /b %ERRORLEVEL%
)
...
start "" powershell.exe -STA -NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File "%~dp0lib\sessions-gui2.ps1" %*
exit /b 0
