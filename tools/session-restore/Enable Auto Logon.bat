...
if not errorlevel 1 (
    powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%~dp0lib\enable-autologon.ps1" %*
    echo.
    pause
    exit /b %ERRORLEVEL%
)
...
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%~dp0lib\enable-autologon.ps1" -Elevate %*
