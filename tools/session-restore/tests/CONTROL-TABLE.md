...
| round-navigation buttons | S5154 `$b.Add_Click({ Invoke-AskMove ([int]$s.Tag) })` | move between the questions of a batched round | UNPROVEN | same `.Tag`-carried-index shape as the option buttons, but `Invoke-AskMove` only moves the panel - it does not send, so it is not policy-blocked. Reads correct; no assertion drives it. |
