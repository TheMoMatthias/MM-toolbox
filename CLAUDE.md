...
- **Mandatory grill gate (replaces the old hook — there is no `UserPromptSubmit` hook anymore):** for anything that doesn't obviously clear the trivial bar above, the first move — before Explore, before drafting any question — is a single selectable `AskUserQuestion`: state your own scope read (light-touch / non-trivial / major / top-tier) as context, then offer to proceed directly, take the light-touch pass, or run the full grill at that tier. The user's click decides; never silently proceed *or* silently grill on their behalf, and never fall back to pattern-matching the prompt's wording for trigger keywords — that approach couldn't tell a task from a question or a discussion and produced repeated false positives, which is why it was removed in favour of this gate.
