...
param(
    ...
    # F-226 HARDENING (measured 2026-08-27, CD chain): the interactive spawn can freeze
    # PRE-TRANSCRIPT on a git-repo target even with a trust entry present (third
    # reproduction) - a live pid, a window, and zero conversation turns forever. With
    # this switch, a spawn whose prompt produced NO transcript turn inside
    # -TurnTimeoutSec is KILLED (our pid, by session id - never a blind name kill) and
    # relaunched HEADLESS (claude -p, same env scrub, fresh session id, hidden window),
    # which the same chain measured working when the interactive path froze. The
    # headless leg gets its own verify window of max(TurnTimeoutSec, 60) seconds.
    # Chain/automation callers should pass this; the interactive default is unchanged.
    [switch]$RetryHeadless,
...
# --- F-226 RETRY LEG: kill the frozen interactive spawn, relaunch headless -----
# Fires only when a prompt was given, no turn landed, and -RetryHeadless was passed.
# The kill targets OUR pid (matched by session id above) and nothing else; if no pid
# ever appeared the window failed outright and there is nothing to kill. The headless
# form is `claude -p` reading the SAME prompt file through the SAME env scrub - the
# path the CD chain measured working when the interactive one froze.
$spawnMode = "interactive"
if ($RetryHeadless -and $fullPrompt -and $turns -eq 0) {
    Write-Host ""
    Write-Host "RETRY      : no conversation turn after ${TurnTimeoutSec}s - killing the frozen spawn and relaunching HEADLESS (F-226)." -ForegroundColor Yellow
    if ($proc) {
        try {
            Stop-Process -Id $proc.ProcessId -Force -ErrorAction Stop
            Write-Host ("RETRY      : killed frozen claude.exe pid " + $proc.ProcessId + " (session " + $sessionId + "). Its window may remain open on a dead shell.")
        } catch {
            Write-Host ("RETRY      : could not kill pid " + $proc.ProcessId + " (" + $_.Exception.Message + ") - proceeding to the headless launch anyway.") -ForegroundColor DarkYellow
        }
    } else {
        Write-Host "RETRY      : no claude.exe ever carried the session id - nothing to kill."
    }

    $sessionId2  = [guid]::NewGuid().ToString()
    $transcript2 = Join-Path $projDir "$sessionId2.jsonl"
    $boot2Path   = Join-Path $bootDir 'boot-headless.ps1'
    $headTokens  = @("--session-id", $sessionId2)
    if (-not [string]::IsNullOrWhiteSpace($Model)) { $headTokens += @("--model", $Model) }
    if ($ClaudeArg.Count -gt 0) { $headTokens += $ClaudeArg }
    $headLiterals = ($headTokens | ForEach-Object { "'" + ($_ -replace "'", "''") + "'" }) -join ', '
    $boot2Lines = @(
        '# Auto-generated by spawn-claude-session (F-226 headless retry leg).',
        'foreach ($v in @(''CLAUDE_CODE_CHILD_SESSION'',''CLAUDE_CODE_SESSION_ID'',''CLAUDECODE'',''CLAUDE_CODE_ENTRYPOINT'',''CLAUDE_PID'',''CLAUDE_CODE_SSE_PORT'')) {',
        '    if (Test-Path ("Env:\" + $v)) { Remove-Item ("Env:\" + $v) -Force }',
        '}',
        ('$claudeArgs = @(' + $headLiterals + ')'),
        ('$promptPath = ' + "'" + ($promptPath -replace "'", "''") + "'"),
        '$p = (Get-Content -Raw -LiteralPath $promptPath).Trim()',
        'claude -p $p @claudeArgs'
    )
    [System.IO.File]::WriteAllLines($boot2Path, [string[]]$boot2Lines, $utf8NoBom)
    Start-Process powershell.exe -ArgumentList @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", ('"' + $boot2Path + '"')) -WorkingDirectory $resolved -WindowStyle Hidden | Out-Null

    $headlessWait = [Math]::Max($TurnTimeoutSec, 60)
    $deadline2 = (Get-Date).AddSeconds($headlessWait)
    while ((Get-Date) -lt $deadline2) {
        Start-Sleep -Milliseconds 750
        if (Test-Path -LiteralPath $transcript2) {
            if (Select-String -LiteralPath $transcript2 -Pattern '"type":"assistant"' -List -Quiet -ErrorAction SilentlyContinue) {
                $turns = 1
                break
            }
        }
    ...
    if ($turns -gt 0) {
        $spawnMode  = "headless-retry"
        $sessionId  = $sessionId2
        $transcript = $transcript2
        Write-Host "RETRY      : headless leg VERIFIED - transcript turn on disk at $transcript2" -ForegroundColor Green
        Write-Host "             The session runs `claude -p` hidden: it works its one turn, then exits."
        Write-Host "             No Remote Control on this leg; resumable AFTER it exits via the resume command below."
    } else {
        Write-Warning "RETRY      : the headless leg ALSO produced no transcript turn within ${headlessWait}s. Both spawn paths failed - see F-226; the chain needs a manual restart."
    }
}
...
Write-Host "Mode       : $spawnMode"
if ($proc -and $spawnMode -eq "interactive")  { Write-Host "VERIFIED   : claude.exe pid $($proc.ProcessId) is running with this session id." }
if ($turns -gt 0) {
...
if ($spawnMode -eq "headless-retry") {
    Write-Host "Remote Control: not applicable to the headless leg (claude -p takes no pairing)."
} elseif ($rcOn) {
...
# --- HONEST EXIT CODE (F-207 discipline: the exit convention IS the thing gated) ---
# A caller automating a chain must be able to branch on this without parsing prose.
# 0 = the spawn is verified working (a transcript turn exists for the prompt given,
#     or no prompt was given so there was nothing to verify).
# 2 = a prompt was given and NO spawn path produced a conversation turn.
if ($fullPrompt -and $turns -eq 0) { exit 2 }
exit 0
