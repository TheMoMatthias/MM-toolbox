...
- **`/ask-me`** — the corrective for a message that decided too much. Use it mid-conversation, inside any other skill, and the agent stops settling things on your behalf: it takes the last response and everything still open as given, groups the open decisions by track, puts them up as selectable questions with a recommendation each, and then **implements** your answers rather than handing back a list. It always returns at least one question. It trusts the session's account of itself — when what you doubt is that account, `/realign` re-checks the claim against evidence first and then does the same thing.
